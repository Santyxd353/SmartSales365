...
export default function AdminOrders(){
  ...
  const search = async (e) => {
    ...
    try {
      ...
      setOrder(data)
    } catch(e){ setError(e?.message || 'No se pudo buscar la orden') }
    finally { setLoading(false) }
  }

  const openReceipt = async (id) => {
    try {
      const res = await fetch(receiptUrl(id), { headers: { 'Authorization': `Bearer ${getAccess()}` } })
    ...
    <section className="container-edge py-8">
      ...
      <form onSubmit={search} className="flex gap-2 mb-4">
        ...
        <button className="btn btn-primary" disabled={loading || !trx.trim()}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M10.5 3a7.5 7.5 0 1 1-4.743 13.36l-3.59 3.59a1.125 1.125 0 1 1-1.592-1.592l3.59-3.59A7.5 7.5 0 0 1 10.5 3Z"/></svg>
      ...
      {order && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="card p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button className="btn btn-primary" onClick={async()=>{
              try{ const updated = await markOrderPaid(order.id, 'simulated'); setOrder(updated); alert('Pago confirmado') }
              catch(e){ alert(e.message) }
            ...
            <button className="btn" onClick={async()=>{
              const reason = prompt('Motivo de anulación', 'Anulación por administración'); if(reason === null) return
              try{ const updated = await voidOrder(order.id, reason); setOrder(updated); alert('Transacción anulada') }
              catch(e){ alert(e.message) }
            }}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M6 18 18 6M6 6l12 12"/></svg> Anular transacción</button>
